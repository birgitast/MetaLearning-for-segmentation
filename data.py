...
def get_datasets(name, folder, num_ways=1, num_shots=1, num_test_shots=None, shuffle=True, seed=None):
    ...
    if name =='Pascal5i':
        ...
        meta_train_dataset = pascal5i(folder, ways=num_ways, shots=num_shots, test_shots=num_test_shots, meta_train=True, transform=transform)#, dataset_transform, class_augmentation, download, fold)
        meta_val_dataset = pascal5i(folder, ways=num_ways, shots=num_shots, test_shots=num_test_shots, meta_split='val', transform=transform)#, dataset_transform, class_augmentation, download, fold)
        meta_test_dataset = pascal5i(folder, ways=num_ways, shots=num_shots, test_shots=num_test_shots, meta_test=True, transform=transform)#, dataset_transform, class_augmentation, download, fold)
